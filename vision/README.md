...
| **[`neurosymbolic-path.md`](neurosymbolic-path.md)** | **A frame for work that already exists** — a typed provenance substrate, a reasoning surface that structurally refuses model calls, and a model-free reader, with no trained weights in the answer path. Names what is missing and what would settle it; the measured case is [`paper-2`](../research/paper-2-knowledge-without-weights.md) | direction |
